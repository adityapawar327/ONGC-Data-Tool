...
def check_and_add_area_column(df, sheet_name=None):
    if 'Area Name / Block' not in df.columns:
        # Insert the new column at position 2 (3rd column, 0-based index)
        df.insert(2, 'Area Name / Block', '')
        
        # Get the number of rows with data in the first column
        data_end_row = df[df.iloc[:, 0].notna()].shape[0]
        
        # Create a text input for the area name with sheet name in the label if provided
        label = f"Enter Area Name / Block value for sheet '{sheet_name}'" if sheet_name else "Enter Area Name / Block value to be applied to all rows"
        area_name = st.text_input(label, "")
        
        if area_name:
            # Apply the entered value to all rows up to where data exists
            df.loc[:data_end_row-1, 'Area Name / Block'] = area_name
    ...
    if uploaded_files and len(uploaded_files) > 0:
        ...
        for file in uploaded_files:
            try:
                ...
                df_file = sheets[selected_sheet]
                
                # Check and add Area Name / Block column if needed
                df_file, column_added = check_and_add_area_column(df_file, selected_sheet)
        ...
        if 'Media_ID' not in combined_df.columns:
            st.warning("The combined data must contain 'Media_ID' column.")
            return
        ...
        if uploaded_file_2:
            ...
            df2 = sheets_2[selected_sheet_2]
            df2.columns = df2.columns.str.strip()
            cols_lower = [c.lower() for c in df2.columns]

            # Check and add Area Name / Block column if needed for second file
            df2, column_added_2 = check_and_add_area_column(df2, selected_sheet_2)
            cols_lower = [c.lower() for c in df2.columns]

            if 'no. of media' not in cols_lower:
                st.warning("The second sheet must contain 'No. of Media' column.")
                return

            media_col = df2.columns[cols_lower.index('no. of media')]

            st.subheader("📄 Preview of Second Uploaded Sheet")
            ...
            second_areas = df2['Area Name / Block'].astype(str).tolist()
            ...
            st.dataframe(df2[['Area Name / Block', 'Matched Area', media_col]], use_container_width=True)
